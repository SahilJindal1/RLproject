...
for run_no in range(num_runs):
    ...
    # theta, episode_lengths = reinforce(env, sigma=0.01, gamma=1., order=4, iterations=iterations, hidden_features=64, alpha=0.001)
    theta, W, episode_lengths = os_actor_critic(env, sigma=0.01, gamma=1., order=4, iterations=iterations, hidden_features_theta=6, hidden_features_w=3, \
                              alpha_theta=0.001, alpha_w=0.0001, render=False)
...
for i in range(to_dipslay):
    generate_episode(env, theta, sigma=0.01, order=4, k=6, render=True)
